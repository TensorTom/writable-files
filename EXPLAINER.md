...
## Secondary Goals/Possibilities

There is much functionality that this API could easily (Not to mean trivially)
branch into. Some of those currently in proposition are:

* Persistent storage permission. A domain/web app would be granted a persistent
handle to users' directories so as to be permitted access when the user is not
engaged in visiting the site. Reference the manner in which Push notifications
behave, for instance.
...
the users entire home directory, or even their entire downloads directory, least
without overt warning to end users.
...
## Extend private data management

Users should have the ability to easily clear data and permissions generated by
this API. Such measures are considerable in reference to super cookies.
